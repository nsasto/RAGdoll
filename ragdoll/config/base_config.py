...
class LoaderConfig(BaseConfig):
    ...
    file_types: List[str] = Field(
        default=[], description="List of file extensions to process"
    )
...
class ChunkerConfig(BaseConfig):
    ...
    chunk_size: int = Field(
        default=1000, description="Size of each chunk in characters"
    )
...
class ClientConfig(BaseConfig):
    ...
    client: str = Field(
        ..., description="The name of the client (e.g., 'openai', 'huggingface')."
    )
    model: str = Field(..., description="The name of the embedding model.")
    kwargs: Dict[str, Any] = Field(
        default_factory=dict, description="Additional settings for the client."
    )
...
class EmbeddingsConfig(BaseConfig):
    ...
    clients: Dict[str, ClientConfig] = Field(
        ..., description="Configurations for each embedding client."
    )
...
class LLMConfig(BaseConfig):
    ...
    model_name: str = Field(
        default="gpt-3.5-turbo", description="Name of the language model to use"
    )
...
class CacheConfig(BaseConfig):
    ...
    """
    cache_ttl: int = Field(
        default=86400, description="Time to live for cached items in seconds."
    )
...
class IngestionConfig(BaseConfig):
    ...
    max_threads: int = Field(
        default=10, description="Maximum threads for concurrent processing"
    )
    batch_size: int = Field(
        default=100, description="Number of documents to process in one batch"
    )
    retry_attempts: int = Field(
        default=3, description="Number of retry attempts for failed ingestion"
    )
    ...
    loaders: LoadersConfig = Field(
        default_factory=LoadersConfig, description="Loaders configuration"
    )
