...
def get_llm(
    ...
    if not is_langchain_available():
        logger.error(
            "LangChain core not installed. Please install with: pip install langchain-core"
        )
        logger.info(
            "You might also need provider-specific packages like langchain-openai, langchain-anthropic, langchain-google-genai."
        )
        return None
    ...
    try:
        ...
        logger.error(
            "Failed to import LangChain chat models. Ensure you have the necessary LangChain integrations installed."
        )
        return None
    ...
    if isinstance(model_name_or_config, str):
        ...
        if model_config is None or provider is None:
            available_models = [
                model.get("model_name") for model in model_list if "model_name" in model
            ]
            logger.error(
                f"Model '{config_model_name}' not found in config or missing provider. Available models: {available_models}"
            )
            return None
    elif isinstance(model_name_or_config, dict):
        model_config = (
            model_name_or_config.copy()
        ...
        logger.error(
            f"Invalid model_name_or_config type: {type(model_name_or_config)}. Expected str or dict."
        )
        return None
    ...
    try:
        logger.debug(
            f"Initializing {provider} model '{actual_model_name}' with config keys: {list(processed_config.keys())}"
        )
        ...
        logger.error(
            f"Failed to initialize {provider} model '{actual_model_name}': {e}"
        )
        return None
...
if __name__ == "__main__":
    ...
    default_gpt = get_llm("gpt-4o-mini")
    if default_gpt:
    ...
    custom_config = {"provider": "openai", "model": "gpt-4o", "temperature": 0.7}
